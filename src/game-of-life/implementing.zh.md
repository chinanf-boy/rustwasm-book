...
生命游戏是在无限的宇宙中进行的，但我们没有无限的记忆和计算能力。 解决这个相当恼人的限制，通常有以下三种风格:
...
JavaScript 运行于[一个`requestAnimationFrame`循环][requestanimationframe]. 在每次迭代中，它将当前的 Universe 绘制到`<pre>`，然后运行`Universe::tick`.
...
内`wasm-game-of-life/www/index.html`，让我们取代`<pre>`我们之前添加了一个`<canvas>`我们将渲染(它也应该在`<body>`， 之前`<script>`加载我们的 JavaScript):
...
为了从 Rust 实现 中获取必要的信息，我们需要为 Universe 的宽度，高度和指向 其单元数组 的指针 添加一些更多的 `getter`函数. 所有这些都暴露在 JavaScript 中.添加这些内容`wasm-game-of-life/src/lib.rs`:
...
接下来，在`wasm-game-of-life/www/index.js`，我们也导入`Cell`从`wasm-game-of-life`，v 让我们在`import`代码行下面定义 JavaScript 在渲染画布时将使用的一些常量:
...
请注意，我们打电话`drawGrid()`和`drawCells()`这里*之前*我们称之为`requestAnimationFrame()`.我们这样做的原因是这样的*初始*在我们进行修改之前绘制宇宙的状态.如果我们改为简单地叫`requestAnimationFrame(renderLoop)`，我们最终得到的情况是绘制的第一帧实际上是*后*第一次打电话给`universe.tick()`这是这些细胞生命的第二个"滴答声".

## 它工作了!

通过在根目录中运行此命令来重建 WebAssembly 和绑定粘合剂`wasm-game-of-life`目录:

```
wasm-pack build
```

确保您的开发服务器仍在运行.如果不是，请从内部再次启动`wasm-game-of-life/www`目录:
...
- 不是硬编码最初的宇宙，而是生成一个随机的，有五十五个细胞活着或死亡的机会.
  ...
  _提示:使用[该`js-sys`箱](https://crates.io/crates/js-sys)进口[该`Math.random`JavaScript 函数](https://developer.mozilla.org/en-US/docs/Web/JavaScript/Reference/Global_Objects/Math/random)._
  ...
  _然后，使用`js_sys::Math::random`翻转硬币的功能:_
...
在 Rust 中，您可以使用[该`fixedbitset`箱子和它的`FixedBitSet`类型](https://crates.io/crates/fixedbitset)代表细胞代替细胞`Vec<Cell>`:
...
要更新宇宙的下一个刻度中的单元格，我们使用`set`的方法`FixedBitSet`:
...
要将指向开头的指针传递给 JavaScript，您可以转换`FixedBitSet`到切片然后将切片转换为指针:
...
在 JavaScript 中，构建一个`Uint8Array`来自 Wasm 的内存与之前相同，只是数组的长度不是`width * height`不过了，但是`width * height / 8`因为我们每位有一个单元而不是每个字节:

```js
const cells = new Uint8Array(memory.buffer, cellsPtr, (width * height) / 8);
```

给出一个索引和`Uint8Array`,你可以确定是否*ñ<sup>日</sup>*使用以下函数设置位:
